...
| 工具 | 版本要求 | 安装方式 |
|------|---------|---------|
| VS Code + PlatformIO IDE 扩展 | 最新版 | https://platformio.org/install/ide?install=vscode |
| Node.js | v16+ | https://nodejs.org/ |
| CH340 / CP2102 串口驱动 | — | 根据开发板 USB 芯片选择对应驱动 |

> macOS 用户：安装驱动后需重启，插入开发板后运行 `ls /dev/cu.*` 确认串口已出现。

---

### 2. 硬件支持

| 开发板 | PlatformIO 环境名称 | Flash | 说明 |
|--------|-------------------|-------|------|
| ESP32-S3-DevKitC-1-N8 | `esp32-s3-devkitc-1-release` | 8MB | **推荐**，已验证 |
| ESP32-DevKitC | `esp32dev-release` | 4MB | 通用款，需确认 Flash 够用 |
| ESP32-C3-DevKitM-1 | `esp32-c3-devkitm-1-release` | 4MB | 低功耗方案 |
| ESP8266 | `esp8266-release` | 4MB | 功能受限，仅供参考 |

Flash 大小要求：固件约 1.35MB，分区方案为 `huge_app.csv`，建议使用 **4MB 及以上** Flash 的开发板。
...
```bash
git clone https://github.com/suifei/esp32kbm.git
cd esp32kbm
```
...
编辑 `platformio.ini`，将 `upload_port` 和 `monitor_port` 改为实际串口地址：

```ini
[env]
upload_port = /dev/cu.usbserial-XXXX   # macOS / Linux
...
- **Windows**：设备管理器 → 端口 (COM & LPT)，记录 COMx 号，填写如 `COM3`
...
```bash
# 确保 pio 已在 PATH 中（默认位于 ~/.platformio/penv/bin/pio）
pio run -e esp32-s3-devkitc-1-release -t upload
```

**方式 B：VS Code 图形界面**

1. 在 PlatformIO 侧边栏选择目标环境（如 `esp32-s3-devkitc-1-release`）
2. 点击 **Upload** 按钮

烧录成功后串口会输出类似 `Duktape initialized` 的日志。

---

### 6. 第四步：蓝牙配对

1. 固件上电后，设备会广播蓝牙名称 **`Smart Keyboard/Mouse`**
2. 在 iPhone / Apple Vision Pro / Mac 的蓝牙设置中找到该设备并点击配对
3. 配对成功后，设备即可接收来自串口的 JS 命令并转为 BLE HID 事件
...
```bash
# 替换 /dev/cu.usbserial-XXXX 为实际串口地址
node click_at.js /dev/cu.usbserial-XXXX 115200
```
...
| 按键 | 功能 |
|------|------|
| 输入 `X Y` 后按 Enter | 输入目标坐标（单位：步长，绝对位移） |
| 空格 Space | 移动到目标坐标并保持悬停 |
| Enter | 在当前位置点击左键 |
| `B` | 停止移动，返回坐标输入 |
| `Q` | 退出程序 |
...
- **串口不可共占**：PlatformIO 串口监视器打开时，`node` 工具无法连接串口，使用前请先关闭监视器。
- **BLE 拥塞**：鼠标移动步长（STEP）建议 ≥ 30，每步等待时间（WAIT）建议 ≥ 20ms，否则可能出现 `notify(): rc=-1` 错误。
- **pio 未找到**：将 `~/.platformio/penv/bin` 添加到 `PATH`：
  ```bash
