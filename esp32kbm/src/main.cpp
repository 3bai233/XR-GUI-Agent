...
static duk_ret_t js_native_rclear(duk_context *ctx) {
  ...
  if (index == -1) {
    memset(runtime_register, 0, sizeof(runtime_register));
...
void js_native_bind() {
  ...
  duk_push_c_function(duk_ctx, js_native_rclear, DUK_VARARGS);
  duk_put_global_string(duk_ctx, "rclear");

  duk_push_c_function(duk_ctx, js_native_rwrite, DUK_VARARGS);
  duk_put_global_string(duk_ctx, "rwrite");

  duk_push_c_function(duk_ctx, js_native_rread, DUK_VARARGS);
  duk_put_global_string(duk_ctx, "rread");

  duk_push_c_function(duk_ctx, js_native_print, DUK_VARARGS);
  duk_put_global_string(duk_ctx, "print");

  duk_push_c_function(duk_ctx, js_native_delay, DUK_VARARGS);
  duk_put_global_string(duk_ctx, "delay");

  duk_push_c_function(duk_ctx, js_native_write, DUK_VARARGS);
  duk_put_global_string(duk_ctx, "write");

  duk_push_c_function(duk_ctx, js_native_read, DUK_VARARGS);
  duk_put_global_string(duk_ctx, "read");

  duk_push_c_function(duk_ctx, js_native_pinMode, DUK_VARARGS);
  duk_put_global_string(duk_ctx, "pin_mode");

  duk_push_c_function(duk_ctx, js_native_ble_check, DUK_VARARGS);
  duk_put_global_string(duk_ctx, "ble_check");

  duk_push_c_function(duk_ctx, js_native_ble_start, DUK_VARARGS);
  duk_put_global_string(duk_ctx, "ble_start");

  duk_push_c_function(duk_ctx, js_native_mouse_move, DUK_VARARGS);
  duk_put_global_string(duk_ctx, "mouse_move");

  duk_push_c_function(duk_ctx, js_native_mouse_move_to, DUK_VARARGS);
  duk_put_global_string(duk_ctx, "mouse_move_to");

  duk_push_c_function(duk_ctx, js_native_mouse_scroll, DUK_VARARGS);
  duk_put_global_string(duk_ctx, "mouse_scroll");

  duk_push_c_function(duk_ctx, js_native_mouse_click, DUK_VARARGS);
  duk_put_global_string(duk_ctx, "mouse_click");

  duk_push_c_function(duk_ctx, js_native_mouse_down, DUK_VARARGS);
  duk_put_global_string(duk_ctx, "mouse_down");

  duk_push_c_function(duk_ctx, js_native_mouse_up, DUK_VARARGS);
  duk_put_global_string(duk_ctx, "mouse_up");

  duk_push_c_function(duk_ctx, js_native_keyboard_print, DUK_VARARGS);
  duk_put_global_string(duk_ctx, "keyboard_print");

  duk_push_c_function(duk_ctx, js_native_keyboard_println, DUK_VARARGS);
  duk_put_global_string(duk_ctx, "keyboard_println");

  duk_push_c_function(duk_ctx, js_native_keyboard_write, DUK_VARARGS);
  duk_put_global_string(duk_ctx, "keyboard_write");

  duk_push_c_function(duk_ctx, js_native_keyboard_press, DUK_VARARGS);
  duk_put_global_string(duk_ctx, "keyboard_press");

  duk_push_c_function(duk_ctx, js_native_keyboard_releaseAll, DUK_VARARGS);
  duk_put_global_string(duk_ctx, "keyboard_releaseAll");
}
//执行js代码
void js_eval(const char *code) {
  ...
  js_native_bind();
  duk_push_string(duk_ctx, code);
  duk_int_t rc = duk_peval(duk_ctx);

  if (rc != DUK_EXEC_SUCCESS) {

    Serial.printf("%s\n", duk_safe_to_stacktrace(duk_ctx, -1));
    canRun = false;

  } else {

    duk_safe_to_string(duk_ctx, -1);
    String res = duk_get_string(duk_ctx, -1);
